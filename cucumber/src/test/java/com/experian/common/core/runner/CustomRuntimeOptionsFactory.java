...
public class CustomRuntimeOptionsFactory {
	...
	private final CustomCucumberOptions customOptions;
	private boolean featuresSpecified = false;
	private boolean glueSpecified = false;
	private boolean pluginSpecified = false;

	public CustomRuntimeOptionsFactory(Class clazz, CustomCucumberOptions customOptions) {
		this.clazz = clazz;
	...
	private void addTags(CucumberOptions options, List<String> args) {
		...
		for (String tags : customOptions.getTags()) {
			args.add("--tags");
			args.add(tags);
		}
	}
	...
	private void addPlugins(CucumberOptions options, List<String> args) {
		...
		for (String plugin : customOptions.getPlugins()) {
			args.add("--plugin");
			args.add(plugin);
			pluginSpecified = true;
		}
	...
	private void addFeatures(CucumberOptions options, List<String> args) {
		...
		if (customOptions != null && customOptions.getFeatures().size() != 0) {
			args.addAll(customOptions.getFeatures());
			featuresSpecified = true;
		}
	}
	...
	private boolean runningInEnvironmentWithoutAnsiSupport() {
		...
		// We will not return intelliJidea as will not color the output when we run the test not in debug;
		return false;
	}
