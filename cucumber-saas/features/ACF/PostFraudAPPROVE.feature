Feature: Post Fraud Decision APPROVE through the REST api
  In order to obtain APPROVE decision for an application based on Fraud and ID services calls,
  As an ACF user,
  I want to send a POST request with  information for one applicant and receive a valid response with PreciseID score and decision APPROVE for the application
