Feature: Post Fraud Decision Decline through the REST api
  In order to obtain Decline decision  for an application based on Fraud and ID services calls,
  As an ACF user,
  I want to send a POST request with  information for one applicant and receive a valid response with Post Fraud decision Decline
