Feature: Final Decision through the REST api
  In order to obtain a final decision  for an application,
  As an ACF user,
  I want to send a POST request with  information for one applicant and receive a valid response with final decision for the application.
  ...
  Scenario: ID:FDSL01 As a User I want to CREATE an application through CLIENT SYSTEM to get final decision APPROVE for the application.
    ...
    Then I verify that the JSON response has fields:
      | $.data.['DV-Results.Result Calls.C3 Policy Rules-Decision Setter Typical Result.Decision Category'] | APPROVE |
  ...
  Scenario: ID:FDSL02 As a User I want to CREATE an application through CLIENT SYSTEM to get final decision REFER for the application as a result of POST- BUREAU decision call.
    ...
    When I send a REST POST request to /v1/applications/TENANT1/NewApp and receive status code HTTP 200
    Then I verify that the JSON response has fields:
      | $.data.['DV-Results.Result Calls.C3 Policy Rules-Decision Setter Typical Result.Decision Category'] | REFER |
