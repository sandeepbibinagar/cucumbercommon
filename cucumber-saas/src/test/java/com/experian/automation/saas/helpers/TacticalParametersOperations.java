...
public class TacticalParametersOperations {
  ...
  public boolean updateParameter(String name, String description, String fromDate, String toDate,
    ...
    for (Integer k = 1; k < data.size(); k++) {
      ...
      for (Integer j = 0; j < data.get(k).size(); j++) {
        ...
        JSONArray jsonAttribute = JsonPath.read(parameterBody.get(1),
                                                "$.*[?(@.name=='" + data.get(0).get(j) + "')].field");
  ...
  public boolean deployParameter(String name, String version) throws UnirestException {
    if (version.equals("LATEST")) {
      ...
      JSONArray versionProperties = JsonPath.read(getParameterVersions(componentId),
                                                  "$.*[?(@.version=='" + version + "')]]");
  ...
  public List<String> getParametersListFromFile(String file) throws IOException {
    ...
    List<String> allParametersList = JsonPath.read(fileParameters.toString(),
                                                   "$.TacticalParameterGroupsData.e.*.meta_datas.name");
    return allParametersList;
  }

  public void updateFromFile(String parameterName, String parameterVersion, String parameterVersionId, String file)
    ...
    if (!nonDeployedParameterVersion.equals(parameterVersion)) {
      throw new IllegalArgumentException(
          "Wrong version: " + parameterVersion + " for parameter " + parameterName + ". Currently deploy-able version: "
              + nonDeployedParameterVersion);
    }
    ...
    JSONObject fileParameters = new XMLOperations().convertXMLToJSON(file);
    JSONArray targetParameterGroup = JsonPath.read(fileParameters.toString(),
                                                   "$.TacticalParameterGroupsData.e.*[?(@.meta_datas.name=='"
                                                       + parameterName + "')]]");

    if (targetParameterGroup.size() != 1) {
      throw new IllegalArgumentException(
          "Parameters with name: " + parameterName + " found: " + targetParameterGroup.size());
    }

    JSONArray paramWeightsNumber = JsonPath.read(targetParameterGroup.toString(),
                                                 "$.*.attributesValues.e[?(@._param_weight)]");
